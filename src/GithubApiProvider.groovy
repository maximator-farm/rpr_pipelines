        ...
        try {
            context.withCredentials([context.string(credentialsId: "githubNotificationAppKey", variable: "GITHUB_APP_KEY"),
                ...
                context.withEnv(["GITHUB_APP_KEY=${context.GITHUB_APP_KEY}"]) {
                    if (context.isUnix()) {
                        installation_token = context.python3("${context.CIS_TOOLS}/auth_github.py --github_app_id ${context.GITHUB_APP_ID} --organization_name ${organization_name} --duration 540", "3.5").split("\n")[-1]
                    } else {
                        installation_token = context.python3("${context.CIS_TOOLS}\\auth_github.py --github_app_id ${context.GITHUB_APP_ID} --organization_name ${organization_name} --duration 540", "35").split("\n")[-1]
                    }
                }
            }
        } catch (Exception e) {
